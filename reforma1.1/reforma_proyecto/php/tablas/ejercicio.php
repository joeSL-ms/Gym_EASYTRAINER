...
$sql_ejercicio= "SELECT * FROM datos GROUP BY ejercicio ORDER BY id_ejercicio";
if($grupo_muscular != 'Todos' && $grupo_muscular){
    $sql_ejercicio= "SELECT * FROM datos where grupo_muscular = '$grupo_muscular' GROUP BY ejercicio ORDER BY id_ejercicio";
}
$datos_ejercicios = mysqli_query($conn,$sql_ejercicio);

while($row = mysqli_fetch_array($datos_ejercicios))
{
    $id_ejer = $row['id_ejercicio'];
